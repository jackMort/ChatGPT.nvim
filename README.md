...
#### `ChatGPT`
`ChatGPT` command which opens interactive window using the `gpt-4`
model.
(also known as `ChatGPT`)

#### `ChatGPTActAs`
`ChatGPTActAs` command which opens a prompt selection from [Awesome ChatGPT Prompts](https://github.com/f/awesome-chatgpt-prompts) to be used with the `gpt-4` model.
...
All the above actions are using `gpt-4` model.
  ...
  "action_name": {
    ...
    "opts": {
      ...
      "params": { # parameters according to the official OpenAI API
        "model": "gpt-4", # or any other model supported by `"type"` in the OpenAI API, use the playground for reference
        "stop": [
